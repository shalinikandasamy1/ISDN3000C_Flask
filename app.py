...
# NEW: latest jpg in local photos folder (no latest.txt)
def get_latest_photo():
    files = [f for f in os.listdir(PHOTO_DIR) if f.endswith(".jpg")]
    if not files:
        return None
    files.sort()              # your filenames are chronological
    return files[-1]          # newest

# Optional helper (unused here) – keep if needed elsewhere
...
# ---- fake capture trigger + buffer game ----

@app.route("/trigger_capture")
def trigger_capture():
    # Real capture happens on RDK via hardware button.
    # Here we just go to the buffer screen while the photo is taken/synced.
...
# SCREEN 3 – Preview (single latest photo only)
@app.route("/preview")
def preview():
...
def choose_confirm(filename):
    # no more writing LATEST_FILE; choosing affects later logic only if you use it
...
def compare():
    ...
    if latest:
        ...
        bw_url = url_for("static", filename=f"../photos_bw/{latest}")
        filtered_url = bw_url
    else:
        original_url = None
        filtered_url = None
